            ...
            <div class="card-body">
                <form action="{{ route('misi.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group mb-2">
                        ...
                        <input type="text" class="form-control" placeholder="Masukan judul misi disini..." name="judul_misi">
                        @error('judul_misi')
                        <small class="text-danger">{{ $message }}</small>@enderror
                    </div>
                    <div class="form-group mb-2">
                        <label for="">Deskripsi misi</label>
                        <textarea name="deskripsi_misi" id="" cols="30" rows="10" class="form-control"></textarea>
                        @error('deskripsi_misi')
                        <small class="text-danger">{{ $message }}</small>@enderror
                    </div>
                    <div class="form-group mb-2">
                        <label for="">Foto <abbr title="" style="color: black">*</abbr> </label>
                        <input id="inputImg" type="file" accept="image/*" name="icon_misi" class="form-control" required />
